        ...
        "items": [
            {
                "time": "12:00",
                "activity": "李記水餃",
                "desc": "哪裡可以領水餃，當舖，因為當舖領(dumplings)",
                "icon": "restaurant",
                "color": "#84A6D3",
            },
        ],
...
async def init_db() -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if count > 0:
            logger.info("已有 %d 筆 trip_templates，跳過 seed", count)
            return
        ...
        logger.info("已 seed %d 筆盲盒行程", len(SEED_TRIPS))
